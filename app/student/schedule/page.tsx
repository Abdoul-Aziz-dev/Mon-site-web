  ...
  return (
    <div className="animate-fade-in">
      <div style={{ marginBottom: '3rem' }}>
        <h1 className="font-heading" style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '-0.04em', marginBottom: '0.5rem' }}>Mon Planning</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', fontWeight: 500 }}>Consultez votre emploi du temps hebdomadaire.</p>
      </div>
              ...
              <div key={day} className="card-premium" style={{ border: '1px solid var(--surface-border)' }}>
                ...
                {daySchedules.length > 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    {daySchedules.map((s) => (
                      <div key={s.id} style={{ padding: '1rem', background: 'var(--surface-hover)', borderRadius: '12px', borderLeft: `4px solid ${dayColor[day] || 'var(--primary)'}` }}>
                        <div style={{ fontWeight: 800, color: 'var(--text-main)', marginBottom: '0.5rem' }}>{s.startTime} - {s.endTime}</div>
                        <div style={{ fontSize: '0.95rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>{s.course.title}</div>
                        <div style={{ fontSize: '0.85rem', color: 'var(--primary)', fontWeight: 700 }}>📍 {s.room || 'Salle TBD'}</div>
                      </div>
                    ))} 
                  </div>
                ) : (
                  <div style={{ padding: '1rem', textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.9rem' }}>Aucun cours</div>
                )}
              </div>
            );
          })}
